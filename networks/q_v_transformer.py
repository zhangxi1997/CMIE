...
class CoAttentionLayer_counter(nn.Module):
    ...
    def forward(
        ...
        attention_qv_counter = torch.zeros_like(attention_qv).uniform_(0, 1) # random
        if scale is not None:
            attention_qv_counter = attention_qv_counter * scale
        if attn_mask is not None:
            attention_qv_counter = attention_qv_counter.masked_fill(attn_mask, -np.inf)
        attention_qv_counter = self.softmax(attention_qv_counter)
        attention_qv_counter = attention_qv_counter.masked_fill(softmax_mask, 0.)
        output_qv_counter = torch.bmm(attention_qv_counter, video)
        output_qv_counter = self.linear_final_qv(output_qv_counter)

        output_q_counter = self.layer_norm_qv(output_qv - output_qv_counter + q)

        attention_vq_counter = torch.zeros_like(attention_vq).uniform_(0, 1)  # random
        if scale is not None:
            attention_vq_counter = attention_vq_counter * scale
        if attn_mask is not None:
            attention_vq_counter = attention_vq_counter.masked_fill(attn_mask_, -np.inf)
        attention_vq_counter = self.softmax(attention_vq_counter)
        attention_vq_counter = attention_vq_counter.masked_fill(softmax_mask_, 0.)
        output_vq_counter = torch.bmm(attention_vq_counter, question)
        output_vq_counter = self.linear_final_vq(output_vq_counter)
        output_v_counter = self.layer_norm_vq(output_vq - output_vq_counter + v)

        return output_q, output_v, output_q_counter, output_v_counter
...
class CoAttention(nn.Module):
    ...
    def __init__(self, hidden_size, n_layers=1, dropout_p=0.0):
        ...
        self.encoder_layers = nn.ModuleList(
            [CoAttentionLayer_counter(hidden_size, dropout_p) for _ in range(n_layers)])

    def forward(self, question, video):
        attn_mask = padding_mask_k(question, video) # video的mask
        softmax_mask = padding_mask_q(question, video)
        attn_mask_ = padding_mask_k(video, question)
        softmax_mask_ = padding_mask_q(video, question)

        for encoder in self.encoder_layers:
            question, video, question_counter, video_counter = encoder(
                question,
                video,
                attn_mask=attn_mask,
                softmax_mask=softmax_mask,
        ...
        return question, video, question_counter, video_counter
